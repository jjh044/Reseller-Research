...
function getMarketplaceCacheKey(brand) {
  return `marketplace:v13:${normalizeBrand(brand)}:${boloLookbackDays}`;
}
...
async function getRapidApiGoogleImageTagReferences(brand) {
  if (!RAPIDAPI_KEY || !RAPIDAPI_GOOGLE_IMAGES_HOST) return [];
...
async function verifyTagReferenceImages(brand, references) {
  if (!OPENAI_API_KEY || references.length === 0) return [];
